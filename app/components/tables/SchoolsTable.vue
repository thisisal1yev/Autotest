...
function getRowItems(row: Row<DrivingSchool>) {
  return [
    ...
    {
      label: "Copy school name",
      icon: "i-lucide-copy",
      onSelect() {
        navigator.clipboard.writeText(row.original.name.toString());
        toast.add({
          title: "Copied to clipboard",
          description: "School name copied to clipboard",
        });
      },
    },
    {
...
const columns: TableColumn<DrivingSchoolWithSubscriptions>[] = [
  ...
  },
  {
    ...
    header: "Plan",
    cell: ({ row }) => {
      const subscriptions = (row.original as any).subscriptions as { plan: string }[] | undefined;
...
<template>
  ...
  <UTable
    ...
    class="shrink-0"
    :data="data as DrivingSchoolWithSubscriptions[]"
    :columns="columns"
